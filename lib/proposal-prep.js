...
export function prepareExecutiveGroupDeal(body, errors) {
  ...
  // Prefer the metros the form sent (chip picker); fall back to the ones
  // found in the CSV so older callers still work.
  const metrosSource = Array.isArray(explicitMetros) && explicitMetros.length
    ? explicitMetros
    : [...new Set(rows.map(r => (r.company_metro_area || '').trim()).filter(Boolean))]
  const LEVEL_RANK = { HIGH: 0, MODERATE: 1, LOW: 2 }
  const selectedMarkets = [...new Set(metrosSource)]
    .map(m => byName.get(m))
    .filter(Boolean)
    .sort((a, b) =>
  ...
  descriptors.push(
    ...
    { kind: 'dynamic', pngKey: 'pricing',   canvasIdx: 11, bg: 'white', footerLogo: true }, // 11 Investment Summary (replaces PPTX slide11.xml)
...
export function preparePartySlateProposalSales(body, errors) {
  ...
  const dt = Math.max(0, Number(discountTier) || 0)
  const dtUnit = discountTierUnit === 'dollar' ? 'dollar' : 'percent'
  if (dtUnit === 'percent' && dt > 100) errors.push('discountTier percent must be 0–100')
  if (errors.length) return null
