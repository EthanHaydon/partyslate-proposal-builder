...
export function preparePartySlateProposalSales(body, errors) {
  ...
  // Standard deck canvas mapping (14 source slides → 13 output):
  // 1 cover · 2-4 photo slides · 5 Stats · 6 Market trends ·
  // 7 CS · 8 Integrations · 9 Standard tier grid · 10 Major tier grid ·
  // 11 Investment Summary EXAMPLE (placeholder — always dropped) ·
  // 12 Extra Value · 13 Trusted · 14 Closer.
  //
  // For cls=standard: dynamic Investment Summary replaces source slide 10
  // (Major tier grid), source 9 (Standard tier grid) kept as static.
  // For cls=major:    dynamic Investment Summary replaces source slide 9
  // (Standard tier grid), source 10 (Major tier grid) kept as static.
  const matchingRefCanvas = cls === 'major' ? 10 : 9
  const pricingTierCanvas = cls === 'major' ? 9 : 10
  ...
  return {
    ...
    descriptors: [
      ...
      { kind: 'static',  sourceIdx: 7,           canvasIdx: 8 },
      // For cls=standard, slot 9 is the matching Standard ref (sourceIdx 8 = source 9)
      // and slot 10 is the dynamic pricing tiers (replacing source slide 10).
      // For cls=major, slot 9 is the dynamic (replacing source 9) and slot 10 is the
      // Major ref (sourceIdx 9 = source 10).
      cls === 'major'
        ? { kind: 'dynamic', pngKey: 'pricingTiers', canvasIdx: pricingTierCanvas, bg: 'white' }
        : { kind: 'static',  sourceIdx: 8,           canvasIdx: matchingRefCanvas },
      cls === 'major'
        ? { kind: 'static',  sourceIdx: 9,           canvasIdx: matchingRefCanvas }
        : { kind: 'dynamic', pngKey: 'pricingTiers', canvasIdx: pricingTierCanvas, bg: 'white' },
      // Source slide 11 (Investment Summary placeholder example) is dropped.
      { kind: 'static',  sourceIdx: 11,          canvasIdx: 12, photoSlot: 3 }, // Extra Value
