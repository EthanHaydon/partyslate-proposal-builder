...
//   discountValue: number — interpreted as a percent or dollar amount
//                  per discountUnit
//   discountUnit:  'percent' | 'dollar'
...
export function renderPricingTiersHtml({
  ...
  const isDollar = discountUnit === 'dollar'
  const groupDiscountAmt = isDollar
    ? Math.min(dv, list)
    : list * (Math.min(dv, 100) / 100)
  ...
  const discountLabel = groupDiscountAmt <= 0
    ? 'None'
    : isDollar
      ? `${fmt(groupDiscountAmt)} Group Discount`
      : `${Math.round((groupDiscountAmt / list) * 100)}% Group Discount`
  const discountRowLabel = isDollar
    ? 'Group Discount'
    : `${Math.round((groupDiscountAmt / list) * 100)}% Group Discount`
...
<body>
  ...
  <div class="right">
    ...
    ${groupDiscountAmt > 0 ? `
    <div class="savings">
      <span>${esc(discountRowLabel)}</span>
