...
export function assemblePptx({ templateBytes, descriptors, renderedPngs, photos }) {
  ...
  for (const [sourceNum, rid] of slideNumToRId) {
    ...
    // Remove the [Content_Types].xml override
    contentTypes = contentTypes.replace(
      new RegExp(`<Override[^>]+PartName="\\/ppt\\/slides\\/slide${sourceNum}\\.xml"[^>]*/>`),
      '',
    )
    // Drop the slide file itself + its rels. Notes slide (if any) is left as
    // a harmless orphan; PowerPoint/Slides ignore unreferenced notes.
    zip.remove(`ppt/slides/slide${sourceNum}.xml`)
    zip.remove(`ppt/slides/_rels/slide${sourceNum}.xml.rels`)
  }
